...
ColorComponent = namedtuple("ColorComponent", "name order verticalSampling horizontalSampling quantizationTableId repeat shape")
...
class JPEGReader:
    def __init__(self, filename:Path):
        ...
        print(f"Reading file '{filename}' ({self.fileSize:,} bytes)")
    ...
    def defineDqt(self, data:bytes) -> None:
        ...
        while dataHeader < dataSize:
            ...
            tableFormat = data[dataHeader] >> 4 # Check wether the QT is stored using 8bit or 16bit values
            dataHeader += 1
            ...
            if tableFormat == 0:
                qtValues = np.array([value for value in data[dataHeader : dataHeader + 64]], dtype = "int8")
                dataHeader += 64
            elif tableFormat != 0:
    ...
    def defineHuffmanTable(self, data:bytes) -> None:
        ...
        while (dataHeader < dataSize):
            ...
            codes_count = {
                bitLength: count
                for bitLength, count
                in zip(range(1, 17), data[dataHeader : dataHeader+16])
            }
            dataHeader += 16
            ...
            for bitLength, count in codes_count.items():
                huffval_dict.update(
                    {bitLength: data[dataHeader : dataHeader+count]}
                )
                dataHeader += count
            ...
            print(f"ID: {tableDestination & 0x0F} ({'DC' if tableDestination >> 4 == 0 else 'AC'})")
    ...
    def startFrame(self, data:bytes) -> None:
        ...
        self.imageHeight = data[dataHeader] << 8 + data[dataHeader + 1] # height could be 0 here, but will be retrievable on DNL segment
        dataHeader += 2
        ...
        if componentsNumber not in (1, 3):
            if componentsNumber == 4:
                ...
                raise UnsupportedJpeg("Error - Unsupported color space (only RDG and Grayscale are)")
        dataHeader += 1
        ...
        try:
            for count, component in enumerate(components, start=1):
                ...
                myComponent = ColorComponent(
                    ...
                    repeat = horizontalSample * verticalSample,           # Amount of times the component repeats during decoding
                    shape = (8*horizontalSample, 8*verticalSample),       # Dimensions (in pixels) of the MCU for the component
                )
        ...
        sampleWidth = max(component.shape[0] for component in self.colorComponents.values())
        sampleHeight = max(component.shape[1] for component in self.colorComponents.values())
        self.sampleShape = (sampleWidth, sampleHeight)

        # Display the samplings
        print(f"Horizontal sampling: {' x '.join(str(component.horizontalSampling) for component in self.colorComponents.values())}")
        print(f"Vertical sampling  : {' x '.join(str(component.verticalSampling) for component in self.colorComponents.values())}")
    ...
    def startScan(self, data:bytes) -> None:
        ...
        for component in range(componentsAmount):
            componentId = data[dataHeader]    # Should match the component ID's on the 'start of frame'
            dataHeader += 1

            # Selector for the Huffman tables
            tables = data[dataHeader]
            dataHeader += 1
            dcTable =  tables >> 4             # Should match the tables ID's on the 'detect huffman table'
        ...
        if self.scanMode == "progressiveDct":
            spectralSelectionStart = data[dataHeader]    # Index of the first values of the data unit
            spectralSelectionEnd = data[dataHeader+1]    # Index of the last values of the data unit
            bitPositionHigh = data[dataHeader+2] >> 4    # The position of the last bit sent in the previous scan
            bitPositionLow = data[dataHeader+2] & 0x0F   # The position of the bit sent in the current scan
        ...
        if self.imageHeight == 0:
            ...
            if dnlIndex != -1:
                ...
                self.imageHeight = (self.__file[dnlIndex+4] << 8) + self.__file[dnlIndex+6]
            else:
                raise CorruptedJpeg("Error - Image height cannot be zero.")

        # Dimensions of the MCU (minimum coding unit)
        if componentsAmount > 1:
            self.mcuWidth:int = 8 * max(component.horizontalSampling for component in self.colorComponents.values())
            self.mcuHeight:int = 8 * max(component.verticalSampling for component in self.colorComponents.values())
        ...
        if componentsAmount > 1:
            self.mcuCount_h = (self.imageWidth // self.mcuWidth) + (0 if self.imageWidth % self.mcuWidth == 0 else 1)
            self.mcuCount_v = (self.imageHeight // self.mcuHeight) + (0 if self.imageHeight % self.mcuHeight == 0 else 1)
        else:
        ...
        if self.imageArray is None:
            ...
            count_h = (self.imageWidth // self.sampleShape[0]) + (0 if self.imageWidth % self.sampleShape[0] == 0 else 1)
            count_v = (self.imageHeight // self.sampleShape[1]) + (0 if self.imageHeight % self.sampleShape[1] == 0 else 1)
            ...
            self.imageArray = np.zeros(shape=(self.arrayWidth, self.arrayHeight, self.arrayDepth), dtype="int16")
        ...
        if self.scanMode == "baselineDct":
            ...
            raise UnsupportedJpeg("Error - Encoding mode not supported. Only 'Baseline DCT' and 'Progressive DCT' are supported.")
    ...
    def baselineDctScan(self, huffmanTablesId:dict, myColorComponents:dict) -> None:
        ...
        def nextHuffval() -> int:
            ...
            while huffmanValue is None:
                codeword += nextBits()
                if len(codeword) > 16:
                    raise CorruptedJpeg(f"Failed to decode image ({currentMcu}/{self.mcuCount} MCUs decoded).")
        ...
        while (currentMcu < self.mcuCount):
            ...
            for depth, (componentId, component) in enumerate(myColorComponents.items()):
                ...
                quantizationTable = self.quantizationTables[component.quantizationTableId]
                ...
                for blockCount in range(repeat):
                    ...
                    dcValue = binTwosComplement(nextBits(huffmanValue)) + previous_dc[depth]
                ...
                self.imageArray[x : x+self.mcuWidth, y : y+self.mcuHeight, component.order] = myMcu
            
            # Go to the next MCU
            currentMcu += 1
            
            # Check for restart interval
            if (self.restartInterval > 0) and (currentMcu % self.restartInterval == 0) and (currentMcu != self.mcuCount):
                nextBits(amount=0, restart=True)
                previous_dc[:] = 0
        self.scanCount += 1

    def progressiveDctScan(self,
        huffmanTablesId:dict,
        myColorComponents:dict,
        spectralSelectionStart:int,
        spectralSelectionEnd:int,
        bitPositionHigh:int,
        bitPositionLow:int) -> None:

        # Whether to the scan contains DC or AC values
        if (spectralSelectionStart == 0) and (spectralSelectionEnd == 0):
            values = "dc"
        elif (spectralSelectionStart > 0) and (spectralSelectionEnd >= spectralSelectionStart):
            values = "ac"
        else:
            raise CorruptedJpeg("Error - Progressive JPEG images cannot contain both DC and AC values in the same scan.")
        """NOTE
        ...
        if bitPositionHigh == 0:
            ...
            raise CorruptedJpeg("Error - Progressive JPEG images cannot contain more than 1 bit for each value on a refining scan.")
        
        print(f"\nScan {self.scanCount+1} of {self.scanAmount}")
        print(f"Color components: {', '.join(component.name for component in myColorComponents.values())}")
        print(f"Spectral selection: {spectralSelectionStart}-{spectralSelectionEnd} ({values.upper()})")
        print(f"Successive approximation: {bitPositionHigh}-{bitPositionLow} ({'refining' if refining else 'first'} scan)")
        print(f"MCU count: {self.mcuCount}")
                ...
                for depth, (componentId, component) in enumerate(myColorComponents.items()):
                    ...
                    for blockCount in range(repeat):
                        ...
                        if not refining:
                            ...
                            self.imageArray[x+delta_x, y+delta_y, component.order] = (dcValue << bitPositionLow)
                        
                        # Refining scan for the DC values
                        else:
                            new_bit = int(nextBits())
                            self.imageArray[x+delta_x, y+delta_y, component.order] |= (new_bit << bitPositionLow)
        ...
        # AC values scan
            ...
            def refine_ac() -> None:
                ...
                while to_refine:
                    ...
                    self.imageArray[ref_x, ref_y, component.order] |= new_bit << bitPositionLow
                    ref_index += 1
            ...
            while (currentMcu < self.mcuCount):
                ...
                while index <= spectralSelectionEnd:  # The element at the end of the band is included
                    ...
                    if acBitsLength > 0:
                        ...
                        if refining:
                            while self.imageArray[x + ac_x, y + ac_y, component.order] != 0:
                                to_refine.append((x + ac_x, y + ac_y))
                                index += 1
                                ac_x, ac_y = zagzig[index]
                        
                        # Create a new acValue
                        self.imageArray[x + ac_x, y + ac_y, component.order] = acValue << bitPositionLow
...
class InverseDCT:
    ...
    for x, y, u, v in xyuv_coordinates:
        ...
        # Frequency component
        idctTable[x, y, u, v] = 0.25 * Cu * Cv * cos((2*x + 1) * pi * u / 16) * cos((2*y + 1) * pi * v / 16)
...
class ResizeGrid:
    ...
    def __call__(self, block:np.ndarray, new_shape:Tuple[int,int]) -> np.ndarray:
        ...
        new_xy = self.meshPoints.get(key)
        if new_xy is None:
